...
| `Qwen3-VL-8B-Instruct-4bit` | VLM 户型图（生产） | `house-vlm-pro` | ✅ **生产默认** |
| `Qwen3-VL-4B-Instruct-4bit` | VLM 基线/回退 | `house-vlm` | ✅ 已有 |
| `bge-m3-mlx-fp16` | Embedding / RAG | `house-embed` | ✅ 已有 |
| `gemma-4-26b-a4b-it-4bit` | 额外文本 | — | ✅ 非必需 |
| `qwen3-0.6b-mlx` | 轻量测试 | — | ✅ 非必需 |

> **已配置：** Admin 中 alias `house-llm` / `house-vlm-pro` / `house-vlm`（回退）/ `house-embed`。
...
| 模型 | 用途 | 当前状态 |
|------|------|----------|
| `Qwen3-VL-8B-Instruct-4bit` | 户型图视觉理解 (VLM) | ✅ **生产默认** → `house-vlm-pro` |
| `Qwen3-VL-4B-Instruct-4bit` | VLM 基线/回退 | ✅ → `house-vlm` |
...
├── Qwen3.5-9B/                   # ✅ 轻量文本
├── Qwen3-VL-8B-Instruct-4bit/    # ✅ VLM 生产（house-vlm-pro）
├── Qwen3-VL-4B-Instruct-4bit/    # 回退（house-vlm）
...
| `Qwen3-VL-8B-Instruct-4bit` | `house-vlm-pro` | 户型图理解（**生产默认**） |
| `Qwen3-VL-4B-Instruct-4bit` | `house-vlm` | 4B 基线/回退 |
| 同上 | `house-vlm-cad` / `house-vlm-mkt` | **可选** A/B；留空则统一用 `house-vlm-pro` |
| `bge-m3-mlx-fp16` | `house-embed` | 向量检索 |

#### 3.4.5 户型 VLM A/B 配置（Phase 5）

后端按图源类型自动选择 VLM alias（`server/.env`）：

```env
HOUSE_DIY_OMLX_VLM_MODEL=house-vlm-pro-pro
HOUSE_DIY_OMLX_VLM_MODEL_CAD=
HOUSE_DIY_OMLX_VLM_MODEL_MARKETING=
```

- `cad_lineart` → `HOUSE_DIY_OMLX_VLM_MODEL_CAD`，未配置则回退 `HOUSE_DIY_OMLX_VLM_MODEL`
- `marketing_color` → `HOUSE_DIY_OMLX_VLM_MODEL_MARKETING`，未配置则回退默认
- 解析结果 `parse_meta.vlm_model` 会落盘，便于排查

**换模型前**可选跑基准脚本（非阻塞）：

```bash
./scripts/benchmark-floorplan-vlm.sh --mock          # 离线
./scripts/benchmark-floorplan-vlm.sh --live          # 真实 oMLX
```

**VLM 配置（2026/05/20）：** 生产解析使用 **`house-vlm-pro`** → `Qwen3-VL-8B-Instruct-4bit`；`house-vlm` 保留给 4B 回退。

Segmentation（M3）配置见 [07-appendix-seg-models.md](./07-appendix-seg-models.md)。

**生产切换原则：** benchmark 房间数、validation、耗时达标后再改 oMLX alias 或 `.env`。
...
- [x] oMLX Admin 中 alias 已配置：`house-llm`、`house-vlm-pro`、`house-vlm`（回退）、`house-embed`
