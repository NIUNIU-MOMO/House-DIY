...
def get_renders_dir(project_id: int, scheme_id: str | None = None) -> Path:
    if scheme_id:
        from app.services.design import scheme_storage

        return scheme_storage.get_renders_dir(project_id, scheme_id)
    from app.services.floorplan.storage import get_project_dir

    return get_project_dir(project_id) / "renders"


def load_manifest(project_id: int, scheme_id: str | None = None) -> RenderManifest:
    path = get_renders_dir(project_id, scheme_id) / MANIFEST_FILENAME
    if not path.is_file():
        return RenderManifest()
    return RenderManifest.model_validate(json.loads(path.read_text(encoding="utf-8")))


def save_manifest(project_id: int, manifest: RenderManifest, scheme_id: str | None = None) -> Path:
    renders_dir = get_renders_dir(project_id, scheme_id)
...
def upsert_render(
    ...
    save_manifest(project_id, manifest, scheme_id)
    return record


def get_render_path(project_id: int, room_id: str, scheme_id: str | None = None) -> Path | None:
    manifest = load_manifest(project_id, scheme_id)
    match = next((item for item in manifest.rooms if item.room_id == room_id), None)
    if match is None:
        return None
    path = get_renders_dir(project_id, scheme_id) / match.filename
